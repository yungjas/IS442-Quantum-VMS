...
export default {
  ...
  methods: {
    ...
    approveForm() {
      ...
      axios
        .put(
          "http://localhost:8080/api/forms/approve/" + approveFormObj.formId,
          approveFormObj,
          {
            headers: {
              "Content-Type": "application/json",
    ...
    unapproveForm() {
      ...
      axios
        .put(
          "http://localhost:8080/api/forms/unapprove/" + approveFormObj.formId,
          approveFormObj,
          {
            headers: {
              "Content-Type": "application/json",
      ...
      for (var i = 0; i < this.placeholderID; i++) {
        ...
        if (element !== null) {
          ...
          for (var j = 0; j < element.length; j++) {
            ...
            var noResponse = element[j].className
              .split(" ")
              .includes("noResponse");
              ...
              if (
                ...
                if (value2.checked) {
                  questionResponse.push({
                    inputName: "radio",
                    inputValue: value2.value,
                  });
                }
                // console.log(questionResponse);
              } else if (
                element[j].type === "checkbox" &&
                element[j].nodeName !== "LABEL"
              ) {
                // console.log(element[j].nodeName)
    ...
    addUserResponse(qnsId, questionResponse, noResponse) {
      ...
      if (noResponse) {
        ...
        axios
          .put(
            "http://localhost:8080/api/response/update/" + formId + "/" + qnsId,
            json,
            {
              headers: {
                "Content-Type": "application/json",
          ...
          if (questionType.toLowerCase() === "text") {
            ...
            if (questionsObj.questionResponse !== undefined) {
              for (var z in questionsObj.questionResponse) {
                inputValueField.setAttribute(
                  "name",
                  questionsObj.questionResponse[z].inputName
                );
                inputValueField.setAttribute(
                  "value",
                  questionsObj.questionResponse[z].inputValue
                );
              }
            }
            ...
            inputValueField.setAttribute(
              "class",
              this.placeholderID + " " + hasQnsResponseClass
            );
            ...
            for (var j in answerChoices) {
              ...
              for (var q in questionsObj.questionResponse) {
                ...
                if (
                  answerChoice !== null &&
                  questionsObj.questionResponse !== undefined
                ) {
                  if (
                    answerChoice.inputValue === questionResponse2.inputValue
                  ) {
            ...
            for (var k in answerChoices2) {
              ...
              for (var l in questionsObj.questionResponse) {
                ...
                if (
                  ...
                  if (
                    answerChoice2.inputValue === questionResponse.inputValue
                  ) {
            ...
            console.log(
              "error: unknown question type for array index" + questions[23]
            );
